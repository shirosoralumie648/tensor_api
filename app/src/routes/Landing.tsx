...
function Feature({ icon, title, desc, className }: { icon: any; title: string; desc: string; className?: string }) {
  return (
    <Card className={`rounded-2xl border border-white/10 bg-white/5 dark:bg-white/5 backdrop-blur-xl hover:border-white/20 hover:shadow-2xl transition-all duration-300 ${className || ""}`}> 
  ...
  return (
    <div className={`min-h-[calc(100vh-64px)] w-full flex flex-col relative overflow-hidden`}> 
      ...
      <section className={`w-full pt-24 pb-16 flex flex-col items-center text-center`}> 
        ...
        <p className={`max-w-2xl leading-6 px-4 text-white/70 reveal`}>
        ...
        <div className={`mt-4 reveal`}> 
          <Badge variant={`secondary`} className={`px-3 py-1 bg-white/10 text-white/80 backdrop-blur`}>开源 · Golang + React + Tauri</Badge>
        </div>
      </section>

      {/* Showcase */}
      <section className={`w-full px-4 sm:px-8 max-w-6xl mx-auto pb-12 reveal`}>
        <div className={`p-[1px] rounded-3xl bg-gradient-to-r from-pink-500 via-fuchsia-500 to-sky-500 shadow-[0_0_40px_-10px_rgba(236,72,153,.6)]`}>
          <div className={`rounded-[22px] relative overflow-hidden backdrop-blur-xl bg-white/5 border border-white/10`}>
            <div className={`aspect-[16/9] w-full`}> 
              ...
              <div className={`relative z-10 h-full w-full p-6 sm:p-10`}>
                <div className={`h-4 w-24 rounded-full bg-white/20 mb-6`} />
                ...
                <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8`}>
                  <div className={`h-24 rounded-xl bg-white/10 backdrop-blur`} />
                  <div className={`h-24 rounded-xl bg-white/10 backdrop-blur`} />
                  <div className={`h-24 rounded-xl bg-white/10 backdrop-blur`} />
                </div>
              </div>
      ...
      <section className={`w-full px-4 sm:px-8 max-w-6xl mx-auto pb-10`}>
        <div className={`text-center mb-6 reveal`}>
          ...
          <p className={`text-white/70 mt-1`}>客服助手 · 编码助手 · 知识检索 · 图像问答</p>
        </div>
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4`}>
          {["客服助手", "编码助手", "知识检索", "图像问答"].map((t, i) => (
            <div key={i} className={`reveal rounded-2xl p-[1px] bg-gradient-to-r from-pink-500/60 via-fuchsia-500/60 to-sky-500/60`}>
              <div className={`rounded-[18px] bg-white/5 backdrop-blur-xl p-5 text-center`}>
                <div className={`text-lg font-semibold`}>{t}</div>
                <div className={`text-sm text-white/70 mt-1`}>一键集成 · 即刻可用</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className={`w-full px-4 sm:px-8 max-w-6xl mx-auto pb-16`}>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4`}>
          {[
            ...
            <div key={i} className={`reveal rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 flex flex-col items-center`}>
              ...
              <div className={`text-sm text-white/70 mt-1 text-center`}>{s.d}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className={`w-full px-4 sm:px-8 max-w-6xl mx-auto pb-10 text-center text-white/60`}> 
        <div className={`text-sm`}>© {new Date().getFullYear()} {appName} · <a className={`underline hover:text-white`} href={docsEndpoint} target={`_blank`}>文档</a></div>
      </footer>
    </div>
  );
}
