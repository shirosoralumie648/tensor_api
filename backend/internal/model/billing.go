...
type Invoice struct {
	ID           int       `gorm:"primaryKey" json:"id"`
	UserID       int       `gorm:"index" json:"user_id"`
	InvoiceNo    string    `gorm:"size:50;uniqueIndex" json:"invoice_no"`   // 发票号
	TotalCost    int64     `json:"total_cost"`                              // 总费用（分）
	TotalUSD     float64   `json:"total_usd"`                               // 总费用（美元）
	ItemCount    int       `json:"item_count"`                              // 项目数量
	Status       int       `gorm:"default:1" json:"status"`                 // 状态: 1=未支付 2=已支付 3=已取消
